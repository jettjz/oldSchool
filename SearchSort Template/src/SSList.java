...
// -----------------------------------------------------------------------------
    ...
    public int RecBinSearch(String s) {
        return RBSCode(s, 0, myList.length-1);
    }

    public int RBSCode(String s, int start, int end) {
        int test = (start + end) / 2;
        if (start >= end) {
            return -1;
        } else if (s.equals(myList[test])) {
            return test;
        } else if (s.compareTo(myList[test]) > 0) {
            return RBSCode(s, test + 1, end);
        } else {
            return RBSCode(s, start, test - 1);
        }
    }
